...
class HtmlEditor extends Component {
  ...
  createElement = async ({ tag }, openContext) => {
    openContext('createElement');
    const id = this.ids.pop();
    this.html.children[0].children.push(this.h(tag, { id }, this.h('text', null, 'oh hello1')));
    const formattedCode = await this.toString();
    this.setState({ val: formattedCode });
  };

  render() {
    return <CodeEditor mode="html" inFocus={this.props.inFocus} val={this.state.val} />;
  }
}
